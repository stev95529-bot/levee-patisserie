...
document.addEventListener('DOMContentLoaded', () => {
  ...
  elements.loginForm.addEventListener('submit', (e) => {
    e.preventDefault();
    isLoggedIn = true;
    localStorage.setItem("usuarioLogueadoNombre", "Usuario Levée");
    localStorage.setItem("usuarioLogueadoCorreo", "cliente@levee.com");
 ...
 document.getElementById('logo-btn').addEventListener('click', function(e) {
    ...
    // Si ya inició sesión antes y abre el CRUD con el logo, asegura que aparezca registrado
    const nombreGuardado = localStorage.getItem("usuarioLogueadoNombre");
    const correoGuardado = localStorage.getItem("usuarioLogueadoCorreo");

    if (nombreGuardado && correoGuardado && typeof registros !== 'undefined' && typeof renderTabla === 'function') {
      const yaExiste = registros.some(r => r.correo === correoGuardado);
      if (!yaExiste) {
        registros.push({
          idEncuesta: String(Math.floor(1000 + Math.random() * 9000)),
          ...
          hora: "12:00",
          genero: "Otro",
          direccion: "Sesión activa",
          direccionBis: "",
          codigoPostal: "110111",
          conocido: "Web",
          satisfaccion: "100",
          comentarios: "Registro automático por sesión activa."
        });
        renderTabla();
      }
    }
...
document.getElementById('closeCrud').addEventListener('click', function() {
